...
# # Send 'clear' and 'cargo build' to the target pane
# # echo -en "clear\r" | wezterm cli send-text --no-paste --pane-id "$target_pane" 2>> /tmp/cargo_build_update.log
# echo -en "cargo build\r" | wezterm cli send-text --no-paste --pane-id "$target_pane" 2>> /tmp/cargo_build_update.log
...
#   echo "Error: Target pane $target_pane is occupied (title: '$pane_title'). Skipping cargo build." >> /tmp/cargo_build_update.log
#   exit 1
# fi
