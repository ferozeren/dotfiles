...
# # Debug: Log arguments
# echo "ck_update.sh called with args: $1" >> /tmp/ck_update.log

# # Get the current pane ID from the argument
# current_pane="$1"
# if [[ -z "$current_pane" ]]; then
#   echo "Error: No current pane ID provided" >> /tmp/ck_update.log
#   exit 1
# fi
...
# if [[ -z "$window_id" ]]; then
#   echo "Error: Failed to get window ID for pane $current_pane" >> /tmp/ck_update.log
#   exit 1
# fi
...
#   echo "Error: No other pane found in window $window_id" >> /tmp/ck_update.log
#   exit 1
# fi

# # Send 'clear' and 'ck run' to the target pane
# # echo -en "clear\r" | wezterm cli send-text --no-paste --pane-id "$target_pane" 2>> /tmp/ck_update.log
# echo -en "ck run\r" | wezterm cli send-text --no-paste --pane-id "$target_pane" 2>> /tmp/ck_update.log
...
#   echo "Error: Target pane $target_pane is occupied (title: '$pane_title'). Skipping ck run." >> /tmp/ck_update.log
#   exit 1
# fi

# # Optional: Additional check for shell prompt (if needed)
# # Send a no-op command (like Enter) to check if the pane is at a prompt
# # echo -en "\r" | wezterm cli send-text --no-paste --pane-id "$target_pane" 2>> /tmp/ck_update.log
...
if [[ ${#pane_ids[@]} -eq 0 ]]; then
  echo "Error: No other panes found in window $window_id" >> /tmp/ck_update.log
  exit 1
fi
...
# Explicitly exclude yazi by checking the title
  if [[ "$pane_title" =~ "bash" ]]; then
    echo "Pane $pane_id is occupied (title: '$pane_title'). Checking next pane." >> /tmp/ck_update.log
    continue
  fi
  ...
  if [[ "$is_idle_shell" == true ]]; then
    ...
    target_pane="$pane_id"
    echo "Found idle pane $target_pane (title: '$pane_title')" >> /tmp/ck_update.log
    break
  else
...
if [[ -z "$target_pane" ]]; then
  echo "Error: No idle panes found in window $window_id" >> /tmp/ck_update.log
  exit 1
fi
