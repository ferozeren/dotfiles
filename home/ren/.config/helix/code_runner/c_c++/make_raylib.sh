...
echo "make_raylib.sh called with args: $1" >> /tmp/make_raylib.log

current_pane="$1"
if [[ -z "$current_pane" ]]; then
  echo "Error: No current pane ID provided" >> /tmp/make_raylib.log
  exit 1
fi
...
if [[ -z "$window_id" ]]; then
  echo "Error: Failed to get window ID for pane $current_pane" >> /tmp/make_raylib.log
  exit 1
fi
...
if [[ ${#pane_ids[@]} -eq 0 ]]; then
  echo "Error: No other panes found in window $window_id" >> /tmp/make_raylib.log
  exit 1
fi

target_pane="${pane_ids[0]}"
echo "Using target pane: $target_pane" >> /tmp/make_raylib.log

# 1. Send Ctrl+C
wezterm cli send-text --pane-id "$target_pane" --no-paste $'\x03' 2>> /tmp/make_raylib.log
sleep 0.2

# 2. Send the command + real Enter (not '\r' string)
wezterm cli send-text --pane-id "$target_pane" --no-paste $"clear; and make reload"$'\r' 2>> /tmp/make_raylib.log
...
#     echo "Pane $pane_id is occupied (title: '$pane_title'). Chemakeing next pane." >> /tmp/make_raylib.log
#     continue
#   fi
...
#     target_pane="$pane_id"
#     echo "Found idle pane $target_pane (title: '$pane_title')" >> /tmp/make_raylib.log
#     break
#   else
...
#   echo "Error: No idle panes found in window $window_id" >> /tmp/make_raylib.log
#   exit 1
# fi

# # Send 'clear' and 'make run' to the target pane
# # echo -en "clear\r" | wezterm cli send-text --no-paste --pane-id "$target_pane" 2>> /tmp/make_raylib.log
# echo -en "make \r" | wezterm cli send-text --no-paste --pane-id "$target_pane" 2>> /tmp/make_raylib.log
